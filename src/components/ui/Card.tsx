...
export default function Card({ children, className = '', accentColor, hover = true }: CardProps) {
  return (
    <div className={`bg-bg-3 border border-border rounded-lg p-3.5 relative overflow-hidden transition-all shadow-inset-border ${hover ? 'hover:border-border-2' : ''} ${className}`}>
      {accentColor && (
        <div
...
export function KpiCard({ label, value, sub, accentColor, className = '', hero = false }: {
  ...
  return (
    <Card accentColor={accentColor} className={`animate-fade-up ${className}`}>
      ...
      {hero ? (
        <div className="hero-num text-[24px] leading-none mono-value text-t-1">{value}</div>
      ) : (
        <div className="hero-num text-[20px] leading-none mono-value text-t-1">{value}</div>
      )}
      {sub && <div className="text-[11px] text-t-3 mt-1.5 font-mono mono-value tracking-tight">{sub}</div>}
    </Card>
  );
}
