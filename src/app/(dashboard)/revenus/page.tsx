...
import { REV_COLORS } from '@/lib/constants';
...
export default function RevenusPage() {
  ...
  // Computed
  const allMonths = Object.keys(rev.months || {}).sort();
  const visibleMonths = curTab === 'all' ? allMonths : [curTab];
  ...
  const openAdd = (month?: string) => {
    ...
    setFormMonth(month || allMonths[allMonths.length - 1] || '');
    setEditIdx(null);
    setAddOpen(true);
  };
  ...
  const pctObj = rev.objectif > 0 ? (totalConfirmed / (rev.objectif * allMonths.length)) * 100 : 0;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-5 max-lg:grid-cols-2 max-md:grid-cols-1">
        <KpiCard label="Confirmés" value={`${f$(totalConfirmed)} €`} sub={`${f0(toAed(totalConfirmed, liveRate))} AED`} accentColor="#10b981" />
        ...
        <div className="bg-bg-3 border border-border rounded-md p-4 relative overflow-hidden">
          ...
          <div className="text-[10px] text-t-3 uppercase tracking-wider font-medium mb-2">Objectif mensuel</div>
          <input
            type="number"
            className="bg-transparent border border-transparent font-mono text-[22px] font-bold w-full outline-none hover:border-border-2 hover:bg-bg-2 focus:border-accent focus:bg-bg-2 rounded-md px-1.5 py-0.5 transition-all mono-value tracking-tight"
            value={rev.objectif}
            onChange={e => updateObjectif(parseFloat(e.target.value) || 0)}
            step="100"
          />
          <div className="text-[11px] text-t-3 mt-1 font-mono">{pctObj.toFixed(0)}% atteint</div>
          <div className="h-[3px] bg-border rounded mt-1 overflow-hidden">
            <div className="h-full bg-purple rounded transition-all duration-300" style={{ width: `${Math.min(pctObj, 100)}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-1 flex-wrap mb-5">
        ...
        {allMonths.map(m => (
          <button key={m} onClick={() => setCurTab(m)} className={`px-3.5 py-1.5 rounded-md text-xs font-semibold transition-all cursor-pointer ${curTab === m ? 'bg-bg-4 text-t-1 border border-border-2' : 'text-t-3 border border-transparent hover:text-t-2'}`}>
            {m}
          </button>
        ))}
      </div>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title={editIdx ? 'Modifier un revenu' : 'Ajouter un revenu'}>
        <div className="space-y-3.5">
          <FormField label="Mois">
            <input className="fi" value={formMonth} onChange={e => setFormMonth(e.target.value)} placeholder="Ex: MAI" />
          </FormField>
          <FormField label="Date">
            <input className="fi" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} placeholder="Ex: 15/05" />
          </FormField>
